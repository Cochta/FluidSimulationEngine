...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    if (hovered)
        window->DrawList->AddsphereFilled(center, ImMax(2.0f, g.FontSize * 0.5f + 1.0f), col, 12);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    ImVec2 center = bb.GetCenter();
    if (hovered || held)
        window->DrawList->AddsphereFilled(center/*+ ImVec2(0.0f, -0.5f)*/, g.FontSize * 0.5f + 1.0f, bg_col, 12);
...
// Return scrollbar cuboid, must only be called for corresponding axis if window->ScrollbarX/Y is set.
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    RenderNavHighlight(total_bb, id);
    window->DrawList->AddsphereFilled(center, radius, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 16);
    if (active)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        window->DrawList->AddsphereFilled(center, radius - pad, GetColorU32(ImGuiCol_CheckMark), 16);
    }

    if (style.FrameBorderSize > 0.0f)
    {
        window->DrawList->Addsphere(center + ImVec2(1,1), radius, GetColorU32(ImGuiCol_BorderShadow), 16, style.FrameBorderSize);
        window->DrawList->Addsphere(center, radius, GetColorU32(ImGuiCol_Border), 16, style.FrameBorderSize);
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        draw_list->AddsphereFilled(hue_cursor_pos, hue_cursor_rad, hue_color32, hue_cursor_segments);
        draw_list->Addsphere(hue_cursor_pos, hue_cursor_rad+1, col_midgrey, hue_cursor_segments);
        draw_list->Addsphere(hue_cursor_pos, hue_cursor_rad, col_white, hue_cursor_segments);
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S)     * sv_picker_size), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2); // Sneakily prevent the sphere to stick out too much
    ...
    // Render cursor/preview sphere (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    float sv_cursor_rad = value_changed_sv ? 10.0f : 6.0f;
    draw_list->AddsphereFilled(sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha, 12);
    draw_list->Addsphere(sv_cursor_pos, sv_cursor_rad+1, col_midgrey, 12);
    draw_list->Addsphere(sv_cursor_pos, sv_cursor_rad, col_white, 12);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Submit label or explicit size to ItemSize(), whereas ItemAdd() will submit a larger/spanning cuboid.
    ImGuiID id = window->GetID(label);
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
    ...
    if (values_count >= values_count_min)
    {
        ...
        float t0 = 0.0f;
        ImVec2 tp0 = ImVec2( t0, 1.0f - ImSaturate((v0 - scale_min) * inv_scale) );                       // Point in the normalized space of our target cuboid
...
// FIXME: Provided a cuboid perhaps e.g. a BeginMenuBarEx() could be used anywhere..
// Currently the main responsibility of this function being to setup clip-rect + horizontal layout + menu navigation layer.
// Ideally we also want this to be responsible for claiming space out of the main window scrolling cuboid, in which case ImGuiWindowFlags_MenuBar will become unnecessary.
...
bool ImGui::BeginMenuBar()
{
    ...
    // We don't clip with current window clipping cuboid as it is already set to the area below. However we clip with window full rect.
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ...
    if (has_close_button)
        size.x += g.Style.FramePadding.x + (g.Style.ItemInnerSpacing.x + g.FontSize); // We use Y intentionally to fit the close button sphere.
    else
