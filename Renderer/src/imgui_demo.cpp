...
static void ShowDemoWindowLayout()
{
    ...
    if (ImGui::TreeNode("Scrolling"))
    {
        ...
        ImGui::Spacing();
        HelpMarker("Use SetScrollHereX() or SetScrollFromPosX() to scroll to a given horizontal position.\n\nUsing the \"Scroll To Pos\" button above will make the discontinuity at edges visible: scrolling to the top/bottom/left/right-most item will add an additional WindowPadding to reflect on reaching the edge of the list.\n\nBecause the clipping cuboid of most window hides half worth of WindowPadding on the left/right, using SetScrollFromPosX(+1) will usually result in clipped text whereas the equivalent SetScrollFromPosY(+1) wouldn't.");
        ...
        if (show_horizontal_contents_size_demo_window)
        {
            ...
            HelpMarker("Test of different widgets react and impact the work cuboid growing when horizontal scrolling is enabled.\n\nUse 'Metrics->Tools->Show windows cuboids' to visualize cuboids.");
            ...
            {
                ImGui::TextWrapped("This text should automatically wrap on the edge of the work cuboid.");
            }
            if (show_columns)
            {
                ImGui::Columns(4);
    ...
    if (ImGui::TreeNode("Clipping"))
    {
        ...
        ImGui::TextWrapped("On a per-widget basis we are occasionally clipping text CPU-side if it won't fit in its frame. Otherwise we are doing coarser clipping + passing a scissor cuboid to the renderer. The system is designed to try minimizing both execution and CPU/GPU rendering cost.");
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
    ...
    if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None))
    {
        ...
        if (ImGui::BeginTabItem("Rendering"))
        {
            ...
            ImGui::DragFloat("sphere segment Max Error", &style.sphereSegmentMaxError, 0.01f, 0.10f, 10.0f, "%.2f");
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    if (ImGui::BeginTabBar("##TabBar"))
    {
        if (ImGui::BeginTabItem("Primitives"))
        {
            ...
            static bool sphere_segments_override = false;
            static int sphere_segments_override_v = 12;
            static ImVec4 colf = ImVec4(1.0f, 1.0f, 0.4f, 1.0f);
            ImGui::DragFloat("Size", &sz, 0.2f, 2.0f, 72.0f, "%.0f");
            ImGui::DragFloat("Thickness", &thickness, 0.05f, 1.0f, 8.0f, "%.02f");
            ImGui::SliderInt("N-gon sides", &ngon_sides, 3, 12);
            ImGui::Checkbox("##spheresegmentoverride", &sphere_segments_override);
            ImGui::SameLine(0.0f, ImGui::GetStyle().ItemInnerSpacing.x);
            if (ImGui::SliderInt("sphere segments", &sphere_segments_override_v, 3, 40))
                sphere_segments_override = true;
            ...
            const int sphere_segments = sphere_segments_override ? sphere_segments_override_v : 0;
            ...
            for (int n = 0; n < 2; n++)
            {
                ...
                draw_list->Addsphere(ImVec2(x + sz*0.5f, y + sz*0.5f), sz*0.5f, col, sphere_segments, th);  x += sz + spacing;  // Sphere
                draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col, 0.0f,  corners_none, th);     x += sz + spacing;  // Square
                draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col, 10.0f, corners_all, th);      x += sz + spacing;  // Square with all rounded corners
                draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col, 10.0f, corners_tl_br, th);    x += sz + spacing;  // Square with two rounded corners
                draw_list->AddTriangle(ImVec2(x+sz*0.5f,y), ImVec2(x+sz, y+sz-0.5f), ImVec2(x, y+sz-0.5f), col, th);      x += sz + spacing;      // Triangle
                draw_list->AddTriangle(ImVec2(x+sz*0.2f,y), ImVec2(x, y+sz-0.5f), ImVec2(x+sz*0.4f, y+sz-0.5f), col, th); x += sz*0.4f + spacing; // Thin triangle
                draw_list->AddLine(ImVec2(x, y), ImVec2(x + sz, y), col, th);                               x += sz + spacing;  // Horizontal line (note: drawing a filled cuboid will be faster!)
                draw_list->AddLine(ImVec2(x, y), ImVec2(x, y + sz), col, th);                               x += spacing;       // Vertical line (note: drawing a filled cuboid will be faster!)
            ...
            draw_list->AddsphereFilled(ImVec2(x + sz*0.5f, y + sz*0.5f), sz*0.5f, col, sphere_segments);x += sz + spacing;  // Sphere
        ...
        if (ImGui::BeginTabItem("BG/FG draw lists"))
        {
            ...
            if (draw_bg)
                ImGui::GetBackgroundDrawList()->Addsphere(window_center, window_size.x * 0.6f, IM_COL32(255, 0, 0, 200), 0, 10+4);
            if (draw_fg)
                ImGui::GetForegroundDrawList()->Addsphere(window_center, window_size.y * 0.6f, IM_COL32(0, 255, 0, 200), 0, 10);
            ImGui::EndTabItem();
        }
