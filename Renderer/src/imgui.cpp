...
/*
 ...
 - 2020/01/22 (1.75) - ImDrawList::Addsphere()/AddsphereFilled() functions don't accept negative radius any more.
 ...
 - 2019/04/29 (1.70) - improved ImDrawList thick strokes (>1.0f) preserving correct thickness up to 90 degrees angles (e.g. cuboids). If you have custom rendering using thick lines, they will appear thicker now.
 ...
 - 2015/12/27 (1.48) - fixed ImDrawList::AddRect() which used to render a cuboid 1 px too large on each axis.
...
#define IMGUI_DEBUG_NAV_RECTS       0   // Display the reference navigation cuboid for each window
...
ImGuiStyle::ImGuiStyle()
{
    ...
    FramePadding            = ImVec2(4,3);      // Padding within a framed cuboid (used by most widgets)
    ...
    AntiAliasedFill         = true;             // Enable anti-aliasing on filled shapes (rounded cuboids, spheres, etc.)
    CurveTessellationTol    = 1.25f;            // Tessellation tolerance when using PathBezierCurveTo() without a specific number of segments. Decrease for highly tessellated curves (higher quality, more polygons), increase to reduce quality.
    sphereSegmentMaxError   = 1.60f;            // Maximum error (in pixels) allowed when using Addsphere()/AddsphereFilled() or drawing rounded corner cuboids with no explicit segment count specified. Decrease for higher quality but more geometry.
...
// Handle clipping on CPU immediately (vs typically let the GPU clip the triangles that are overlapping the clipping cuboid edges)
void ImGui::RenderTextClippedEx(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    if (clip_rect) // If we had no explicit clipping cuboid then pos==clip_min
...
// This is made more complex because we have dissociated the layout cuboid (pos_min..pos_max) which define _where_ the ellipsis is, from actual clipping of text and limit of the ellipsis display.
...
// Render a cuboid shaped with optional rounding and borders
...
ImGuiID ImGuiWindow::GetIDFromcuboid(const ImRect& r_abs)
{
    ImGuiID seed = IDStack.back();
...
void ImGui::NewFrame()
{
    ...
    g.DrawListSharedData.SetsphereSegmentMaxError(g.Style.sphereSegmentMaxError);
...
// with SetWindowPos() and not SetNextWindowPos() will have that cuboid lagging by a frame at the time FindHoveredWindow() is
...
// Test if mouse cursor is hovering given cuboid
// NB- cuboid is clipped by our current clip setting
// NB- Expand the cuboid to be generous on imprecise inputs systems (g.Style.TouchExtraPadding)
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // At this point we don't have a clipping cuboid setup yet, so we can use the title bar area for hit detection and drawing
        ...
        // UPDATE cuboidS (1- THOSE NOT AFFECTED BY SCROLLING)
        // Update various regions. Variables they depends on should be set above in this function.
        // We set this up after processing the resize grip so that our cuboids doesn't lag by a frame.
        ...
        // Inner clipping cuboid.
        // Will extend a little bit outside the normal work region.
        // This is to allow e.g. Selectable or CollapsingHeader or some separators to cover that space.
        // Force round operator last to ensure that e.g. (int)(max.x-min.x) in user's render code produce correct result.
        // Note that if our window is collapsed we will end up with an inverted (~null) clipping cuboid which is the correct behavior.
        ...
        // Setup draw list and outer clipping cuboid
        ...
        // Draw navigation selection/windowing cuboid background
        ...
        // Draw navigation selection/windowing cuboid border
        ...
        // UPDATE cuboidS (2- THOSE AFFECTED BY SCROLLING)
        ...
        // Apply focus (we need to call FocusWindow() AFTER setting DC.CursorStartPos so our initial navigation reference cuboid can start around there)
        if (want_focus)
        {
            FocusWindow(window);
...
void ImGui::End()
{
    ...
    if (window->DC.CurrentColumns)
        EndColumns();
    PopClipRect();   // Inner window clip cuboid
...
// Content size = inner scrollable cuboid, padded with WindowPadding.
...
static void ImGui::ErrorCheckNewFrameSanityChecks()
{
    ...
    IM_ASSERT(g.Style.sphereSegmentMaxError > 0.0f                      && "Invalid style setting!");
...
void ImGui::ItemSize(const ImVec2& size, float text_baseline_y)
{
    ...
    //if (g.IO.KeyAlt) window->DrawList->Addsphere(window->DC.CursorMaxPos, 3.0f, IM_COL32(255,0,0,255), 4); // [DEBUG]
...
bool ImGui::ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg)
{
    ...
    // We need to calculate this now to take account of the current clipping cuboid (as items like Selectable may change them)
    if (IsMouseHoveringRect(bb.Min, bb.Max))
...
// r_avoid = the cuboid to avoid (e.g. for tooltip it is a cuboid around the mouse cursor which we want to avoid. for popups it's a small point around the cursor.)
// r_outer = the visible area cuboid, minus safe area padding. If our popup size won't fit because of safe area padding we ignore it.
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    // We perform scoring on items bounding box clipped by the current clipping cuboid on the other axis (clipping on our movement axis would give us equal scores for all clipped items)
...
static void ImGui::NavUpdate()
{
    ...
    {
        // Forwarding previous request (which has been modified, e.g. wrap around menus rewrite the requests with a starting cuboid at the other side of the window)
    ...
    if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.NavWindowingTarget)
    {
        ...
        // Next movement request will clamp the NavId reference cuboid to the visible area, so navigation will resume within those bounds.
    ...
    if (g.NavWindow)
    {
        ...
        if (1) { ImU32 col = (!g.NavWindow->Hidden) ? IM_COL32(255,0,255,255) : IM_COL32(255,0,0,255); ImVec2 p = NavCalcPreferredRefPos(); char buf[32]; ImFormatString(buf, 32, "%d", g.NavLayer); draw_list->AddsphereFilled(p, 3.0f, col); draw_list->AddText(NULL, 13.0f, p + ImVec2(8,-4), col, buf); }
    }
#endif
}
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        if (source_id == 0)
        {
            ...
            source_id = window->DC.LastItemId = window->GetIDFromcuboid(window->DC.LastItemRect);
...
// 1) we use LastItemRectHoveredRect which handles items that pushes a temporarily clip cuboid in their code. Calling BeginDragDropTargetCustom(LastItemRect) would not handle them.
...
bool ImGui::BeginDragDropTarget()
{
    ...
    if (id == 0)
        id = window->GetIDFromcuboid(display_rect);
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawCmdShowMeshAndBoundingBox(ImGuiWindow* window, const ImDrawList* draw_list, const ImDrawCmd* draw_cmd, int elem_offset, bool show_mesh, bool show_aabb)
        {
            ...
            if (show_aabb)
            {
                fg_draw_list->AddRect(ImFloor(clip_rect.Min), ImFloor(clip_rect.Max), IM_COL32(255, 0, 255, 255)); // In pink: clipping cuboid submitted to GPU
    ...
    if (ImGui::TreeNode("Tools"))
    {
        ...
        ImGui::Checkbox("Show windows cuboids", &show_windows_rects);
        ImGui::SameLine();
    ...
    // Overlay: Display windows cuboids and Begin Order
...
#ifdef IMGUI_HAS_TABLE
    // Overlay: Display Tables cuboids
