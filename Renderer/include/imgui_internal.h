...
struct ImRect;                      // An axis-aligned cuboid (2 points)
...
struct IMGUI_API ImRect
{
    ...
    void        ClipWith(const ImRect& r)           { Min = ImMax(Min, r.Min); Max = ImMin(Max, r.Max); }                   // Simple version, may lead to an inverted cuboid, which is fine for Contains/Overlaps test but not for display.
...
// ImDrawList: Helper function to calculate a sphere's segment count given its radius and a "maximum error" value.
#define IM_DRAWLIST_sphere_AUTO_SEGMENT_MIN                     12
#define IM_DRAWLIST_sphere_AUTO_SEGMENT_MAX                     512
#define IM_DRAWLIST_sphere_AUTO_SEGMENT_CALC(_RAD,_MAXERROR)    ImClamp((int)((IM_PI * 2.0f) / ImAcos(((_RAD) - (_MAXERROR)) / (_RAD))), IM_DRAWLIST_sphere_AUTO_SEGMENT_MIN, IM_DRAWLIST_sphere_AUTO_SEGMENT_MAX)
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    float           sphereSegmentMaxError;      // Number of sphere segments to use per pixel of radius for Addsphere() etc
    ImVec4          ClipRectFullscreen;         // Value for PushClipRectFullscreen()
    ImDrawListFlags InitialFlags;               // Initial flags at the beginning of the frame (it is possible to alter flags on a per-drawlist basis afterwards)

    // [Internal] Lookup tables
    ImVec2          ArcFastVtx[12 * IM_DRAWLIST_ARCFAST_TESSELLATION_MULTIPLIER];  // FIXME: Bake rounded corners fill/borders in atlas
    ImU8            sphereSegmentCounts[64];    // Precomputed segment count for given radius (array index + 1) before we calculate it dynamically (to avoid calculation overhead)

    ImDrawListSharedData();
    void SetsphereSegmentMaxError(float max_error);
};
...
struct ImGuiContext
{
    ...
    ImRect                  NavScoringRect;                     // cuboid used for scoring, in screen space. Based of window->DC.NavRefRectRel[], modified for directional navigation scoring.
    ...
    ImRect                  DragDropTargetRect;                 // Store cuboid of current target candidate (we favor small targets when overlapping)
...
struct IMGUI_API ImGuiWindow
{
    ...
    // The best way to understand what those cuboids are is to use the 'Metrics -> Tools -> Show windows cuboids' viewer.
    // The main 'OuterRect', omitted as a field, is window->Rect().
    ImRect                  OuterRectClipped;                   // == Window->Rect() just after setup in Begin(). == window->Rect() for root window.
    ImRect                  InnerRect;                          // Inner cuboid (omit title bar, menu bar, scroll bar)
    ...
    ImRect                  ClipRect;                           // Current clipping/scissoring cuboid, evolve as we are using PushClipRect(), etc. == DrawList->clip_rect_stack.back().
    ...
    ImRect                  NavRectRel[ImGuiNavLayer_COUNT];    // Reference cuboid, in window relative space
...
public:
    ...
    ImGuiID     GetIDFromcuboid(const ImRect& r_abs);
