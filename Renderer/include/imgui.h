...
namespace ImGui
{
    ...
    IMGUI_API void          Bullet();                                                       // draw a small sphere and keep the cursor on the same line. advance cursor x position by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
    ...
    IMGUI_API ImVec2        GetItemRectMin();                                                   // get upper-left bounding cuboid of the last item (screen space)
    IMGUI_API ImVec2        GetItemRectMax();                                                   // get lower-right bounding cuboid of the last item (screen space)
    IMGUI_API ImVec2        GetItemRectSize();                                                  // get size of last item
    IMGUI_API void          SetItemAllowOverlap();                                              // allow last item to be overlapped by a subsequent item. sometimes useful with invisible buttons, selectables, etc. to catch unused area.

    // Miscellaneous Utilities
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // test if cuboid (of given size, starting from cursor position) is visible / not clipped.
    IMGUI_API bool          IsRectVisible(const ImVec2& rect_min, const ImVec2& rect_max);      // test if cuboid (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_AcceptNoDrawDefaultRect      = 1 << 11,  // Do not draw the default highlight cuboid when hovering over target.
...
enum ImGuiNavInput_
{
    ...
    ImGuiNavInput_Cancel,        // cancel / close / exit                        // e.g. Sphere (PS4), B (Xbox), B (Switch), Escape (Keyboard)
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_PickerHueBar    = 1 << 25,  // [Picker]     // ColorPicker: bar for Hue, cuboid for Sat/Value.
...
enum ImGuiMouseCursor_
{
    ...
    ImGuiMouseCursor_NotAllowed,        // When hovering something with disallowed interaction. Usually a crossed sphere.
...
struct ImGuiStyle
{
    ...
    ImVec2      FramePadding;               // Padding within a framed cuboid (used by most widgets).
    ...
    bool        AntiAliasedFill;            // Enable anti-aliasing on filled shapes (rounded cuboids, spheres, etc.)
    float       CurveTessellationTol;       // Tessellation tolerance when using PathBezierCurveTo() without a specific number of segments. Decrease for highly tessellated curves (higher quality, more polygons), increase to reduce quality.
    float       sphereSegmentMaxError;      // Maximum error (in pixels) allowed when using Addsphere()/AddsphereFilled() or drawing rounded corner cuboids with no explicit segment count specified. Decrease for higher quality but more geometry.
...
struct ImDrawCmd
{
    ...
    ImVec4          ClipRect;               // Clipping cuboid (x1, y1, x2, y2). Subtract ImDrawData->DisplayPos to get clipping cuboid in "viewport" coordinates
...
struct ImDrawList
{
    ...
    // - For sphere primitives, use "num_segments == 0" to automatically calculate tessellation (preferred).
    //   In future versions we will use textures to provide cheaper and higher-quality spheres.
    ...
    IMGUI_API void  Addsphere(const ImVec2& center, float radius, ImU32 col, int num_segments = 12, float thickness = 1.0f);
    IMGUI_API void  AddsphereFilled(const ImVec2& center, float radius, ImU32 col, int num_segments = 12);
    ...
    // - "p_min" and "p_max" represent the upper-left and lower-right corners of the cuboid.
    ...
    IMGUI_API void  PathArcToFast(const ImVec2& center, float radius, int a_min_of_12, int a_max_of_12);                                            // Use precomputed angles for a 12 steps sphere
    ...
    // - Use to minimize draw calls (e.g. if going back-and-forth between multiple clipping cuboids, prefer to append into separate channels then merge at the end)
    ...
    IMGUI_API void  PrimRect(const ImVec2& a, const ImVec2& b, ImU32 col);      // Axis aligned cuboid (composed of two triangles)
...
struct ImFontAtlasCustomRect
{
    ...
    unsigned short  Width, Height;  // Input    // Desired cuboid dimension
...
struct ImFontAtlas
{
    ...
    //-------------------------------------------
    // [BETA] Custom cuboids/Glyphs API
    //-------------------------------------------

    // You can request arbitrary cuboids to be packed into the atlas, for your own purposes.
    // After calling Build(), you can query the cuboid position and render your pixels.
    // You can also request your cuboids to be mapped as font glyph (given a font + Unicode point),
    ...
    IMGUI_API int               AddCustomRectFontGlyph(ImFont* font, ImWchar id, int width, int height, float advance_x, const ImVec2& offset = ImVec2(0,0));   // Id needs to be < 0x110000 to register a cuboid to map into a specific font.
    ...
    ImVector<ImFontAtlasCustomRect> CustomRects;    // cuboids for packing custom texture data into the atlas.
    ImVector<ImFontConfig>      ConfigData;         // Internal data
    int                         CustomRectIds[1];   // Identifiers of custom texture cuboid used by ImFontAtlas/ImDrawList
